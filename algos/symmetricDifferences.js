...
function symmetricDifferences(numsA, numsB) {
    ...
    for (let i = 0; i <numsA.length; i++){
        // check if the current element is inside of the second array, if is not then push into results array
        //alert(fruits.includes("Banana")); // Outputs: true
        if (numsB.includes(numsA[i])!== true && results.includes(numsA[i]) !== true) {
            results.push(numsA[i])
        } 
    }

    for (let i = 0; i <numsB.length; i++){
        if (numsA.includes(numsB[i]) !== true && results.includes(numsB[i]) !== true){
            results.push(numsB[i])
        }
    }
    return results
